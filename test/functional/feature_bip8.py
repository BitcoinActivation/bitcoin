...
class BIP8Test(BitcoinTestFramework):
    ...
    def apply_blocks_and_check(self, versions, base_height, state, height=None):
        ...
        # always active
        assert_equal(status[1], {'type': 'bip8', 'bip8': {'status': 'active', 'startheight': 0, 'timeoutheight': 0, 'lockinontimeout': True, 'since': 0}, 'height': 0, 'active': True})
        ...
        if state[0] == "defined":
            ...
            assert_equal(status[2], {'type': 'bip8', 'bip8': {'status': 'started', 'bit': 28, 'startheight': start, 'timeoutheight': stop, 'lockinontimeout': False, 'since': since, 'statistics': None}, 'active': False})
        elif state[0] == "locked_in":
            assert_equal(status[2], {'type': 'bip8', 'bip8': {'status': 'locked_in', 'bit': 28, 'startheight': start, 'timeoutheight': stop, 'lockinontimeout': False, 'since': state[1], 'statistics': None}, 'active': False})
        elif state[0] == "active":
        ...
        if state[0] == "defined":
            ...
            assert_equal(status[3], {'type': 'bip8', 'bip8': {'status': 'started', 'bit': 28, 'startheight': start, 'timeoutheight': stop, 'lockinontimeout': True, 'since': state[1], 'statistics': None}, 'active': False})
        elif state[0] == "must_signal" or state[0] == "stopped":
            since = state[1] - state[1] % 144
            assert_equal(status[3], {'type': 'bip8', 'bip8': {'status': 'must_signal', 'bit': 28, 'startheight': start, 'timeoutheight': stop, 'lockinontimeout': True, 'since': since, 'statistics': None}, 'active': False})
        elif state[0] == "locked_in":
            assert_equal(status[3], {'type': 'bip8', 'bip8': {'status': 'locked_in', 'bit': 28, 'startheight': start, 'timeoutheight': stop, 'lockinontimeout': True, 'since': state[1], 'statistics': None}, 'active': False})
        elif state[0] == "active":
    ...
    def setup_vbparams(self):
        ...
        self.start_nodes(extra_args = [
            # never active
            ["-vbparams=testdummy:@0:@0:0"],
            # always active
            ["-vbparams=testdummy:@0:@0:1"],
